...
func (circuit *hintCircuit) Define(api frontend.API) error {
	res, err := api.Compiler().NewHint(solver.NewHint("ith_bit", bits.IthBit), 1, circuit.A, 3)
	if err != nil {
		return fmt.Errorf("IthBit circuitA 3: %w", err)
	}
	a3b := res[0]
	res, err = api.Compiler().NewHint(solver.NewHint("ith_bit", bits.IthBit), 1, circuit.A, 25)
	if err != nil {
		return fmt.Errorf("IthBit circuitA 25: %w", err)
	}
	a25b := res[0]

	res, err = api.Compiler().NewHint(solver.NewHint("inv_zero", solver.InvZeroHint), 1, circuit.A)
	if err != nil {
		return fmt.Errorf("IsZero CircuitA: %w", err)
	}
	aInvZero := res[0]

	res, err = api.Compiler().NewHint(solver.NewHint("inv_zero", solver.InvZeroHint), 1, circuit.B)
	if err != nil {
